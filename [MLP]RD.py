...
from MLP.MLP import common_MLP
import time
...
# 将空间坐标与时间坐标配对
time_points_broadcasted = time_points.repeat_interleave(X_flatten.shape[0])
X_repeated = X_flatten.repeat(num_time_points)
Y_repeated = Y_flatten.repeat(num_time_points)

u_true, v_true = analytical_solution(X_repeated, Y_repeated, time_points_broadcasted)
...
# 正确调整 coordinates_time 的维度与 time_grid 匹配
coordinates_time = coordinates.repeat(num_time_points, 1)
time_grid = time_points.repeat_interleave(coordinates.size(0)).unsqueeze(1).to(device)
...
writer.add_text('[MLP]Training/Duration', f'Total training time: {training_duration:.2f} seconds')

with open("[MLP_RD]Training.txt", "w") as f:
    for elapsed_time in time_logs:
        f.write(f"{elapsed_time:.6f} seconds\n")

with open("[MLP_RD]Accuracy.txt", "w") as f:
...
plt.colorbar()
plt.title('[MLP_RD]u-Component Velocity Field')
plt.xlabel('X Position')
plt.ylabel('Y Position')
plt.axis('equal')
plt.tight_layout()
plt.savefig('[MLP_RD]u_Component.png', dpi=300)
plt.close()
...
plt.colorbar()
plt.title('[MLP_RD]v-Component Velocity Field')
plt.xlabel('X Position')
plt.ylabel('Y Position')
plt.axis('equal')
plt.tight_layout()
plt.savefig('[MLP_RD]v_Component.png', dpi=300)
plt.close()
...
plt.colorbar()
plt.title('[MLP_RD]Velocity Magnitude Contour')
plt.xlabel('X Position')
plt.ylabel('Y Position')
plt.axis('equal')
plt.tight_layout()
plt.savefig('[MLP_RD]Velocity_Magnitude_Contour.png', dpi=300)
plt.close()

# 保存损失曲线图像
plt.figure(figsize=(10, 5))
plt.plot(losses)
plt.title('[MLP_RD]Training Loss over Time')
plt.xlabel('Steps')
plt.ylabel('Loss')
plt.grid(True)
plt.tight_layout()
plt.savefig('[MLP_RD]Training_Loss.png', dpi=300)
plt.close()
