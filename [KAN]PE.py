...
# Model setup
model = KAN(width=[3,6,6,1], grid=10, k=3, grid_eps=1.0, noise_scale=0.10).to(device)
...
def poisson_residuals(coords):
    ...
    bc_loss = torch.mean((phi_pred[0, :] - 0) ** 2)  # Example boundary condition
...
writer.add_text('[KAN_PE]Training/Duration', f'Total training time: {training_duration:.2f} seconds')

with open("[KAN_PE]Training.txt", "w") as f:
    for elapsed_time in time_logs:
        f.write(f"{elapsed_time:.6f} seconds\n")

with open("[KAN_PE]Accuracy.txt", "w") as f:
    for loss in losses:
        f.write(f"{loss:.6f}\n")
...
plt.colorbar()
plt.title('[KAN_PE]Potential Field Distribution')
plt.xlabel('X Position')
plt.ylabel('Y Position')
plt.axis('equal')
plt.tight_layout()
plt.savefig('[KAN_PE]Potential_Field_Distribution.png', dpi=300)
plt.close()

# Plot training loss
plt.figure(figsize=(10, 5))
plt.plot(losses)
plt.title('[KAN_PE]Training Loss over Time')
plt.xlabel('Steps')
plt.ylabel('Loss')
plt.grid(True)
plt.tight_layout()
plt.savefig('[KAN_PE]Training_Loss.png', dpi=300)
plt.close()
...
ax.set_title('[KAN_PE]3D Potential Field Distribution')
ax.set_xlabel('X Position')
ax.set_ylabel('Y Position')
ax.set_zlabel('Potential')
plt.tight_layout()
plt.savefig('[KAN_PE]3D_Potential_Field_Distribution.png', dpi=300)
plt.close()
