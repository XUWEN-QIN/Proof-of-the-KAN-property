...
class KANLayer(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=2, num=5, k=3, noise_scale=0.1, scale_base=1.0, scale_sp=1.0, base_fun=torch.nn.SiLU(), grid_eps=0.02, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True, save_plot_data = True, device=device, sparse_init=False):
        ''''
        initialize a KANLayer
    ...
    def forward(self, x):
        ...
        #改过的
        preacts = x[:, None, :, :].clone().expand(batch, self.out_dim, timesteps, features)
        # 上面扩展四个维度张量 [batch, 1, timesteps, features] -> [batch, out_dim, timesteps, features]
        postacts = self.base_fun(preacts)
