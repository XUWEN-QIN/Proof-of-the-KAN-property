...
# 加载 .mat 文件
data = scipy.io.loadmat('predictions.mat')
a = data['a']
u = data['u']
t = data['t']
...
def plot_time_slices(data, steps, height, width):
    ...
    for i, time_step in enumerate(steps):
        ax = axs[i]
        ax.imshow(data[time_step].reshape(height, width), cmap='jet', aspect='equal')
...
def plot_2d_heatmap(data, steps, height, width):
    ...
    for i, time_step in enumerate(steps):
        ax = axs[i]
        im = ax.imshow(data[time_step].reshape(height, width), cmap='hot', aspect='equal')
...
# 设置要绘制的指定时间步
specified_steps = [11,12]  # 可以输入任意的时间步
...
# 使用原始数据进行绘图
plot_time_slices(original_data, specified_steps, u.shape[2], u.shape[3])
plot_2d_heatmap(original_data, specified_steps, u.shape[2], u.shape[3])
plot_vector_field(original_data, specified_steps, u.shape[2], u.shape[3])
plot_3d_vector_field(original_data, specified_steps, u.shape[2], u.shape[3])
plot_3d_surface_and_contour(original_data, specified_steps, u.shape[2], u.shape[3])
