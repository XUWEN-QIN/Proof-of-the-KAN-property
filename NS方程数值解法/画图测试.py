...
# 常量设置
initial_lr = 0.002  # 初始学习率
epochs = 1  # 训练迭代次数
batch_size = 832  # 16GB显存建议值
l2_lambda = 1e-5  # L2 正则化系数
...
def plot_3d_surface_and_contour(data, steps, height, width):
    x, y = np.meshgrid(np.linspace(0, 1, width), np.linspace(0, 1, height))
    fig, axs = plt.subplots(4, 5, subplot_kw={'projection': '3d'}, figsize=(20, 14))
    for i, time_step in enumerate(steps):
        ax = axs[i // 5, i % 5]
...
# 调试时检查形状
print(f"in drawing,Original data shape: {original_data.shape}")
