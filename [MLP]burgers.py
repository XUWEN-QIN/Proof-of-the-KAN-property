...
def analytical_solution(x, y, t):
    ...
    p_true = torch.zeros_like(x)  # 假设压力场为0
...
def train():
    ...
    for step in pbar:
        ...
        # Calculate accuracy for current step
        with torch.no_grad():
...
# Write elapsed times to a file
with open("[MLP_burgers]Training.txt", "w") as f:
    for elapsed_time in time_logs:
        f.write(f"{elapsed_time:.6f} seconds\n")

# Write accuracies to a file
with open("[MLP_burgers]Accuracy.txt", "w") as f:
...
save_plot(magnitude, f'[MLP_burgers]Velocity Magnitude Contour at t={t[time_step]:.2f}', '[MLP_burgers]Velocity Magnitude Contour.png')
save_plot(u_pred[time_step].cpu().numpy(), f'[MLP_burgers]u-Component Velocity Field at t={t[time_step]:.2f}', '[MLP_burgers]u-Component Velocity Field.png', cmap='coolwarm')
save_plot(v_pred[time_step].cpu().numpy(), f'[MLP_burgers]v-Component Velocity Field at t={t[time_step]:.2f}', '[MLP_burgers]v-Component Velocity Field.png', cmap='coolwarm')
save_plot(p_pred[time_step].cpu().numpy(), f'[MLP_burgers]Pressure Field Distribution at t={t[time_step]:.2f}', '[MLP_burgers]Pressure Field Distribution.png', cmap='coolwarm')

plt.figure(figsize=(10, 5))
plt.plot(losses)
plt.title('[MLP_burgers]Training Loss')
plt.xlabel('Steps')
plt.ylabel('Loss')
plt.grid(True)
plt.tight_layout()
plt.savefig('[MLP_burgers]Training Loss.png', dpi=300)
plt.close()
